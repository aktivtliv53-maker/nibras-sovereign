...
# الإصدار: المعايرة الكبرى - كروت الاستنطاق وثبات الطاقة
...
# ==============================================================================
st.set_page_config(page_title="Nibras v27.9-Final", page_icon="🛡️", layout="wide")
...
<style>
    ...
    /* كروت الاستنطاق - حل مشكلة عمود الحروف */
    .insight-card {
...
def get_sovereign_gene(root_name, original_weight):
    ...
    if val >= 190: return "G", val
    if val >= 185: return "C", val
    if val >= 180: return "B", val
    if val >= 170: return "S", val
    if val > 80: return "G", val
    if val > 60: return "C", val
    if val > 40: return "B", val
    return "S", val
...
def display_insight_cards(bodies):
    """عرض الاستنطاق ككروت - حل مشكلة عمود الحروف"""
    if not bodies:
        return
    for res in bodies:
        st.markdown(f"""
...
with st.sidebar:
    ...
    <div>
        ...
        <p>🐪 الإبل: {len([r for r in r_index.values() if r['gene'] == 'C'])}</p>
        <p>🐄 البقر: {len([r for r in r_index.values() if r['gene'] == 'B'])}</p>
        <p>🐑 الضأن: {len([r for r in r_index.values() if r['gene'] == 'S'])}</p>
        <p>🐐 المعز: {len([r for r in r_index.values() if r['gene'] == 'G'])}</p>
    </div>
    ---
    <p>خِت فِت.</p>
    """, unsafe_allow_html=True)
...
with tabs[2]:
    ...
    if st.session_state.orbit_active and st.session_state.orbit_bodies:
        ...
        col1.plotly_chart(px.pie(df, names='gene', color='gene', color_discrete_map={g: GENE_STYLE[g]['color'] for g in GENE_STYLE}, hole=0.5))
        col2.plotly_chart(px.bar(df.groupby('gene').size().reset_index(name='count'), x='gene', y='count', color='gene', color_discrete_map={g: GENE_STYLE[g]['color'] for g in GENE_STYLE}))
        st.plotly_chart(px.scatter(df, x='root', y='energy', color='gene', size='energy', color_discrete_map={g: GENE_STYLE[g]['color'] for g in GENE_STYLE}))
    else:
...
with tabs[5]:
    ...
    if st.session_state.orbit_active and st.session_state.orbit_bodies:
        ...
        <div class="story-box">
            ...
            عدد الجذور: {len(bodies)}<br>
            مجموع الطاقة: {total_e:.1f}<br>
            الهيمنة: {GENE_STYLE[dom_gene]['icon']} {GENE_STYLE[dom_gene]['name']}
        </div>
        """, unsafe_allow_html=True)
        display_insight_cards(bodies)
    else:
