...
# --- استدعاء محركات الإدراك (بروتوكول v20.3.2 الصارم) ---
try:
    ...
    st.error(f"⚠️ نقص في ملفات المحركات المساعدة: {e}")

# =========================================================
# 1) التهيئة والقاموس الوجودي (The Sovereign Core)
# =========================================================
st.set_page_config(page_title="Nibras v20.3.2 Sovereign Fusion", page_icon="🎙️", layout="wide")
...
<style>
    ...
    .fusion-sig { color: #4CAF50; font-family: 'Courier New', monospace; font-size: 0.85em; }
</style>
""", unsafe_allow_html=True)

# =========================================================
# 2) محركات التحليل الهيكلي (Data & Internal Logic)
# =========================================================
...
# =========================================================
# 3) الواجهة والتنفيذ (The Sovereign Path Cycle)
# =========================================================
...
if roots_data:
    ...
    st.title("🎙️ محراب نبراس v20.3.2 - الاندماج السيادي")
        ...
        if any(all_res):
            ...
            with tab1:
                ...
                for i, s_list in enumerate(all_res):
                    if s_list:
                        with cols[i]:
                            # --- v20.3.2: الضبط المعياري للمتغيرات (root_energies) ---
                            ...
                            # استدعاء المحرك المدمج مع تصحيح الـ NameError
                            ...
                            with st.expander("🧬 بصمة الحرف والمدار"):
                                ...
                                st.write(f"⚡ متوسط الرنين: **{round(fusion['avg_energy'], 2)}**")
                                st.divider()
                                for item in fusion["profile"]:
                                    ...
                                    sig_val = round(item.get('fused_energy', 0) / (fusion['avg_energy'] or 1), 2)
                                    st.caption(f"قوة الرنين المداري: {sig_val}")

            with tab3:
                # البيان الوجودي الموحد المصفى
                global_state = detect_state([dom_root])
                u_arc = UNIVERSAL_ARCHETYPES.get(Q_ARCHETYPES.get(dom_root, "بناء"), "The Architect")
                
                field_stmt = f"المحور الدلالي المركزي: **'{dom_root}'**. النمط الحاكم: **{u_arc}**."
                state_stmt = f"الحالة السيادية للمنظومة: **{global_state}**."
                
                st.markdown(f"""
                <div class='eloquence-box'>
                    {purify_text(field_stmt)}<br><br>
                    {purify_text(state_stmt)}<br><br>
                    <b>الخلاصة:</b> نبراس يرصد الآن تدفقاً في مقام '{u_arc}' بحالة '{global_state}'.
                </div>
                """, unsafe_allow_html=True)
...
st.sidebar.write("v20.3.2 Sovereign Fusion | خِت فِت.")
