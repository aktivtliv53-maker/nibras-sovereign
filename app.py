...
# الإصدار: Sovereign Gene Expansion v27.6 - الحوكمة الجينية الصارمة
...
def get_sovereign_gene(root_name, original_weight):
    """
    محرك الحوكمة الجينية (v27.6-Final Correction)
    يتم تجاهل الوزن الرقمي إذا وجد الجذر في القوائم السيادية لضمان دقة الأنعام.
    """
    # تطهير وتحضير الجذر للمطابقة الدقيقة
    r = normalize_sovereign(root_name).strip()
    
    # 1. قائمة الإبل (🐪): طاقة المسير، الصبر، السعي، والتمكين البعيد
    camel_roots = ["سفر", "صبر", "بعث", "سعي", "جمل", "روح", "وجه", "طلع", "سير", "نشر", "قصد", "سبل"]
    
    # 2. قائمة البقر (🐄): طاقة التثبيت، الرزق، الوفرة المادية، والأرض
    cow_roots = ["رزق", "رزاق", "ارض", "ثبت", "زرع", "نبت", "طعم", "بني", "مكث", "كنز", "مال", "حيا"]
    
    # 3. قائمة الضأن (🐑): طاقة السكينة، اللين، الرحمة، واليسر
    sheep_roots = ["يسر", "سلم", "رحم", "لين", "رفق", "امن", "هدي", "نور", "سكن", "وضع", "شرح"]
    
    # 4. قائمة المعز (🐐): طاقة السيادة، الحِدّة، الصعود، والأمر القاطع
    goat_roots = ["علو", "قدر", "قهر", "ملك", "حكم", "امر", "سما", "رفع", "عزز", "سلط", "حق", "فصل"]

    # --- بروتوكول الحسم السيادي (الأولوية للهوية) ---
    if r in camel_roots:
        return "C", 65.0  # تثبيت الإبل
    if r in cow_roots:
        return "B", 52.0  # تثبيت البقر (تم حل مشكلة الرزق هنا)
    if r in sheep_roots:
        return "S", 35.0  # تثبيت الضأن
    if r in goat_roots:
        return "G", 95.0  # تثبيت المعز

    # --- معالجة الأوزان الرقمية لبقية الجذور (Scale Correction) ---
    # إذا كان الوزن قادماً من الملف برقم عشري (مثل 1.84) أو مئوي (مثل 184)
    w = float(original_weight)
    if w > 100: 
        w = 90.0  # سقف طاقي لمنع هيمنة المعز العشوائية
    elif w < 1.0: 
        w = w * 100  # تحويل الأرقام العشرية الصغيرة لنظام المئة
    
    # التوزيع الرقمي التلقائي
    if w > 80: 
        return "G", w
    if w > 60: 
        return "C", w
...
def load_semantic_roots_db(path):
    ...
    for orbit_block in data:
        ...
        for item in orbit_block.get("roots", []):
            ...
            # استخدام محرك الحوكمة الجينية v27.6
...
**الإصدار:** v27.6-Final (Sovereign Gene Governance)  
**CPU:** السجدة (5)  
---
**خِت فِت.**
""")
